...
exports.SocketConstructor = Socket;
...
Socket.prototype.send = function(msg, flags) {
  if (Array.isArray(msg)) {
    for (var i = 0, len = msg.length; i < len; i++) {
      this.send(this.encode(msg[i]), i < len - 1
        ? zmq.ZMQ_SNDMORE
        : flags);
    }
  } else {
  ...
  try {
    while (true) {
      ...
      if (flags & zmq.ZMQ_POLLIN) {
          ...
          do {
            emitArgs.push(this.decode(this._zmq.recv()));
...
/**
 * Encoding function, called for values passed to send().
 *
 * @return {Buffer} or {String}
 * @api public
 */

Socket.prototype.encode = function (buf) {
  return buf;
};

/**
 * Decoding function, called for values received by on('message').
 *
 * @return {Buffer} or {String}
 * @api public
 */
